...
export default function GamePage({ params }: { params: Promise<{ gameId: string }> }) {
  ...
  useEffect(() => {
    ...
    const homeButtonCookie = document.cookie.split('; ').find(row => row.startsWith('showHomeButton='));
    if (homeButtonCookie) {
      setShowHomeButton(homeButtonCookie.split('=')[1] !== 'false');
    }
  }, []);
    ...
    fetch(`https://cdn.jsdelivr.net/gh/gn-math/assets@master/zones.json?v=${Date.now()}`)
      ...
      .then(data => {
        ...
        if (game) {
          ...
          setIframeUrl(`${gameUrl}?v=${Date.now()}`);
          setLoading(false);
          return null;
        } else {
